...
## Bước 3.5: Goal-backward verification + Cross-phase integration check

### 3.5a — Kiểm tra tiêu chí thành công từng phase (verification 4 cấp)
...
3. **Nếu phase đã có VERIFICATION_REPORT.md** (từ write-code Bước 9.5):
   ...
   - Nếu `Đạt` → phase ✅, bỏ qua re-verify
   - Nếu `Có gap` hoặc `Cần kiểm tra thủ công` → re-verify bên dưới (code có thể đã sửa sau đó)
4. Nếu CHƯA có VERIFICATION_REPORT hoặc cần re-verify → thực hiện verification 4 cấp (xem @references/verification-patterns.md):
   - **Cấp 1 — Tồn tại**: Đọc "Artifacts" → Glob verify file tồn tại trên đĩa
   - **Cấp 2 — Thực chất**: Kiểm tra nội dung thật (không stub) — scan anti-patterns trong @references/verification-patterns.md, kiểm tra cột "Kiểm tra tự động" nếu PLAN.md có
   - **Cấp 3 — Kết nối**: Đọc "Key Links" → Grep verify import/export/gọi hàm còn nguyên
   - **Cấp 4 — Truths**: Đọc "Cách kiểm chứng" → verify logic/test tồn tại
5. Tổng hợp:
...
3. Nếu PLAN.md các phases KHÔNG có Key Links cross-phase → skip, chỉ kiểm tra basic:
   ...
   - Verify file output của phase trước tồn tại và được import đúng ở phase sau
...
- Nếu có vấn đề → **CẢNH BÁO** (không block):
  > "Phát hiện [X] vấn đề trong goal verification / integration check.

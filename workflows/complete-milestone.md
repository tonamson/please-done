...
### 3.5a — Kiểm tra tiêu chí thành công từng phase
...
3. Nếu CÓ → kiểm tra từng Truth:
   - Đọc "Artifacts" → verify file tồn tại trên đĩa (Glob)
   - Đọc "Key Links" → verify liên kết còn nguyên (Grep import/export/gọi hàm)
   - Đọc "Cách kiểm chứng" → verify logic/test tồn tại
4. Tổng hợp:
   - Phase đạt 100% Truths → ✅
   - Phase có Truth chưa đạt → ⚠️ liệt kê

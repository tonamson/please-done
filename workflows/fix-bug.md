<purpose>
Coordinate 5 investigation agents to find and fix bugs using the scientific method: collect symptoms (Janitor) -> analyze code + documentation (Detective + DocSpec) -> create reproduction test (Repro) -> synthesize and render verdict (Architect) -> fix code and commit (Orchestrator directly).
Save investigation state (.planning/debug/) to resume when conversation is lost. Loop until user confirms.
</purpose>
...
- @references/context7-pipeline.md -> library documentation lookup via Context7 — WHEN bug is related to a library
</conditional_reading>
...
<success_criteria>
- [ ] Session created/resumed correctly via session-manager.js
- [ ] Janitor collected symptoms and wrote evidence_janitor.md
- [ ] Detective and DocSpec ran independently, results merged via parallel-dispatch.js
- [ ] Repro Engineer created reproduction test (or WARNING if failed)
- [ ] Architect synthesized evidence and rendered 1 of 3 verdicts
- [ ] Outcome routing handled all 3 cases correctly
- [ ] Fix applied, tests pass, committed with fix([BUG]) format
- [ ] Bug record created AFTER user verify, INDEX.md updated
- [ ] Session closed with status=resolved AFTER bug record
- [ ] Logic sync ran non-blocking after verify
- [ ] Each step showed progressive disclosure banner

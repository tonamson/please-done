...
Khi user gọi `$pd-fix-bug {{args}}`, thực hiện toàn bộ instructions bên dưới.
## Tool mapping
...
- Có phiên tiếp tục được VÀ không có {{GSD_ARGS}} → liệt kê (trạng thái + giả thuyết + việc tiếp):
...
- Có {{GSD_ARGS}} → tiếp tục thu thập mới
...
Thu đủ 5 thông tin — từ {{GSD_ARGS}} hoặc hỏi:
...
{{GSD_ARGS}} đủ → KHÔNG hỏi thêm, tự trích xuất.

...
Khi user gọi `$pd-write-code {{args}}`, thực hiện toàn bộ instructions bên dưới.
## Tool mapping
...
| `{{GSD_ARGS}}` chỉ định task | ⬜/🔄 → tiếp tục. ✅ → hỏi thực hiện lại? ❌ → hỏi xác nhận → đổi ❌→🔄. 🐛 → "Chạy `$pd-fix-bug`." |

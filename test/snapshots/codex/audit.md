...
Run this step only when PTES-related work applies: `parsePtesFlags({{PD_ARGS}})` yields `recon`, `poc`, or `redteam` true (e.g. `--recon`, `--recon-light`, `--recon-full`, `--poc`, `--redteam`). If all are false and tier is `none`, **skip** Step 0 and go to Step 2.
1. Parse flags (project root):
   ...
   ```
   Pass `{{PD_ARGS}}` via env in wrapper, or inline: substitute the arguments string for the parser.
...
4. **Run Reconnaissance:** If `parsed.recon === true` and tier !== 'none':
   ...
   b. Call: `await runRecon(projectPath, {{PD_ARGS}})`

...
Lấy danh sách 13 valid slugs bằng execute:
```bash
...
Không có flag → chạy smart selection (toàn bộ bước này KHÔNG spawn AI — chỉ execute/glob/search):
1. **Thu thập project context:**
   ...
   d. search code patterns (3-4 lệnh gom nhóm):
...
4. agent pd-sec-scanner dùng FastCode tool-first, search fallback (per D-14 Phase 46)

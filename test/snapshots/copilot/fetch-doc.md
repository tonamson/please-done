...
- **Library version:**
  ...
  2. search for that name in `**/package.json` (excluding node_modules) -> get the version
...
fetch the URL -> extract content + internal links on the same domain (technical pages only, skip blog/changelog)
**HTTP errors:**
...
- Rank up to 10 pages -> fetch the top 5 pages (multiple fetch calls in one block)
...
**Common errors:**
- fetch is unavailable -> check MCP

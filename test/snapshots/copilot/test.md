...
Dùng `fastcode/code_qa` (repos: đường dẫn từ CONTEXT.md):
- "Endpoint [X] làm gì? Request/response format? Validations? Error cases?"
- Ưu tiên đọc code thực tế (FastCode/search) — PLAN.md chỉ để kiểm tra compliance, KHÔNG phải source-of-truth cho test
- FastCode MCP lỗi → Fallback search/read. Warning: "FastCode MCP lỗi — dùng built-in tools. Chạy `/pd:init` kiểm tra lại."
...
- FastCode MCP lỗi → fallback search/read, ghi warning gợi ý `/pd:init`

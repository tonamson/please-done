...
`.pdconfig` -> `SKILLS_DIR`
(Claude Code: `~/.copilot/.pdconfig` -- nền tảng khác: tự chuyển đổi)
</context>
<execution_context>
...
- `rm -f ~/.copilot/cache/pd-update-check.json`
## Bước 9: Thông báo
```
+--------------------------------------+
|     CẬP NHẬT THÀNH CÔNG!            |
...
- Xóa `~/.copilot/cache/pd-update-check.json`
...
- PHẢI xóa `~/.copilot/cache/pd-update-check.json` sau khi cập nhật

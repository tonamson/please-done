...
```
- question not available as tool → ask as plain text (per rules). User does not respond OR tool technical error → auto backup: `.planning/milestones/` → `.planning/milestones_backup_[DD_MM_YYYY]/`. Note: "Auto-backed up due to no response received."
**`--reset-phase-numbers`:**
...
- question not available → ask as plain text, wait for response
</rules>

...
- Hiển thị `question`: 1 vấn đề → single select. 2-4 → multiSelect. 5+ → hỏi phạm vi trước, chia nhóm ≤4
### 3.5.2: Thảo luận từng vấn đề
Với MỖI vấn đề đã chọn:
1. Tiêu đề + bối cảnh (viết cho người KHÔNG phải dev)
2. question single select: option đầu = recommend "(Đề xuất)", tối đa 4 + Other
...
- DISCUSS: question cho mọi lựa chọn. Không khả dụng → văn bản thường (KHÔNG gõ A/B/C)

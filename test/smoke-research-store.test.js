...
describe("Constants", () => {
  ...
  it("REQUIRED_FIELDS gom 5 truong bat buoc", () => {
    assert.deepStrictEqual(REQUIRED_FIELDS, [
      "agent",
      "created",
      "source",
      "topic",
...
| 2026-03-25T10:00:00.000Z | collector | collect | topic1 | 2 | HIGH |`;
    ...
    const headerCount = (result.match(/\| Timestamp \| Agent \| Action/g) || [])
      .length;
...
describe("generateIndex — co entries", () => {
  it("1 entry => table voi 1 row (dung format index-generator)", () => {
    ...
    assert.ok(
      result.includes("| File | Source Type | Topic | Confidence | Created |"),
    );
...
describe("routeQuery — internal patterns", () => {
  it("file extension .ts trong cau hoi -> internal", () => {
    assert.equal(
      routeQuery("ham createUser trong user.service.ts"),
      "internal",
    );
  });
  ...
  it("camelCase function name -> internal", () => {
    assert.equal(
      routeQuery("validateConfidence hoat dong the nao"),
      "internal",
    );
  });
});
...
describe("createEntry — claims rendering", () => {
  ...
  it("createEntry voi claims[] render inline confidence tags trong ## Evidence", () => {
    const result = createEntry({
      ...
      claims: [
        {
          text: "API dung JWT tokens",
          source: "Source code auth.js",
          confidence: "HIGH",
        },
        {
          text: "Session timeout 30 phut",
          source: "Config docs",
          confidence: "MEDIUM",
        },
      ],
  ...
  it("createEntry voi body da co ## Evidence + claims => khong duplicate header", () => {
    ...
    assert.ok(
      result.content.includes(
        "- New claim \u2014 NewSource (confidence: HIGH)",
      ),
    );
  });

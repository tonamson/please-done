...
describe('formatLogEntry', () => {
  ...
  it('throw khi thieu timestamp', () => {
    assert.throws(() => formatLogEntry({
      agent: 'test', action: 'create', topic: 'test',
    }), /thieu truong bat buoc: timestamp/);
  });

  it('throw khi thieu agent', () => {
    assert.throws(() => formatLogEntry({
      timestamp: '2026-03-25T12:00:00Z', action: 'create', topic: 'test',
    }), /thieu truong bat buoc: agent/);
  });

  it('throw khi entry null', () => {
    assert.throws(() => formatLogEntry(null), /entry khong hop le/);
  });

  it('throw khi entry khong phai object', () => {
    assert.throws(() => formatLogEntry('not-object'), /entry khong hop le/);
  });
});
...
describe('appendLogEntry', () => {
  ...
  it('throw khi entry thieu truong bat buoc', () => {
    assert.throws(() => appendLogEntry(createAuditLog(), {
      agent: 'test', action: 'create',

...
} = require("../bin/lib/generate-diagrams");
...
describe("generateBusinessLogicDiagram — subgraph splitting", () => {
  it(">15 Truths triggers subgraph grouping", () => {
    ...
    assert.ok(
      result.diagram.includes("subgraph"),
      "should use subgraphs for >15 truths",
    );
    // Subgraph labels should be in English
    assert.ok(
      result.diagram.includes("Plan 01") || result.diagram.includes("Plan"),
      "subgraph labels should be in English",
    );
  });
});
...
describe("generateBusinessLogicDiagram — validation", () => {
  it("output passes mermaidValidator", () => {
    ...
    assert.equal(
      validation.valid,
      true,
      `validator errors: ${JSON.stringify(validation.errors)}`,
    );
  });
  ...
  it("Vietnamese labels are double-quoted", () => {
    ...
    const unquotedRect = result.diagram.match(
      /\b[A-Za-z_]\w*\[(?!"|(\[|"))[^\]]+\]/g,
    );
    assert.equal(
      unquotedRect,
...
describe("generateArchitectureDiagram — milestone scoping", () => {
  it("only filesModified appear as nodes", () => {
    ...
    assert.ok(
      !result.diagram.includes("Converter"),
      "Converter Layer should be excluded",
    );
  });
});
...
describe("generateArchitectureDiagram — shapes", () => {
  it("bin/lib files get rectangle, templates get subroutine", () => {
    ...
    assert.ok(
      result.diagram.includes('["'),
      "should contain rectangle shape for bin/lib files",
    );
    // Subroutine for external: [["label"]]
    assert.ok(
      result.diagram.includes('[["'),
      "should contain subroutine shape for templates/references files",
    );
  });
});
...
describe("generateArchitectureDiagram — subgraphs", () => {
  it("each active layer becomes a subgraph", () => {
    ...
    assert.ok(
      subgraphCount >= 2,
      `expected >= 2 subgraphs, got ${subgraphCount}`,
    );
  });
});
...
describe("generateArchitectureDiagram — validation", () => {
  it("output passes mermaidValidator", () => {
    const codebaseMaps = { architecture: makeArchitectureMd() };
    const planMeta = {
      filesModified: ["bin/lib/utils.js", "templates/plan.md"],
    };

...
describe("Security agent files", () => {
  ...
  it("security-fix-phase.md template ton tai va co 4 sections", () => {
    ...
    const content = readFileSync(path, "utf8");
    assert.ok(content.includes("## Original Evidence"));
    assert.ok(content.includes("## Gadget Chain"));
    assert.ok(content.includes("## Fix Approach"));
    assert.ok(content.includes("## Completion Criteria"));
  });
});

...
describe("createBugRecord", () => {
  ...
  it("Test 4: Body chua rootCause va fix text", () => {
    ...
    assert.ok(result.content.includes("Root cause: Bien null"));
...
describe("searchBugs", () => {
  ...
  it("Test 14: Error message matching la case-insensitive substring ca 2 chieu", () => {
    ...
    assert.ok(
      m1.length >= 1,
      "TYPEERROR phai khop TypeError: cannot read null",
    );

    // Nguoc lai
    const bug4 = makeBugRecord({
      id: "BUG-004",
      number: 4,
      file: "other.py",
      func: "other",
  ...
  it("Test 17: Thieu tat ca tieu chi tra ve warnings", () => {
    ...
    assert.ok(
      warnings.some((w) => w.includes("all search criteria are empty")),
    );
  });
...
describe("buildIndex", () => {
  ...
  const bug2 = makeBugRecord({
    ...
    file: "src/db.js",
    func: "queryData",
    errorMessage: "ConnectionError: timeout",
    sessionId: "S002",
    status: "resolved",
  ...
  it('Test 19: bugRecords=[] tra ve markdown voi "Tong so: 0 bugs"', () => {
    ...
    assert.ok(result.includes("Total:**"), "must have Total");
    assert.ok(result.includes("0 bugs"), "must have 0 bugs");
  });

  it('Test 20: Voi 2 bug records, INDEX co section "## Theo File" voi bang', () => {
    const result = buildIndex([bug1, bug2]);
    assert.ok(result.includes("## By File"));
  ...
  it('Test 21: Voi 2 bug records, INDEX co section "## Theo Function" voi bang', () => {
    const result = buildIndex([bug1, bug2]);
    assert.ok(result.includes("## By Function"));
  ...
  it('Test 22: Voi 2 bug records, INDEX co section "## Tat ca Bugs" voi bang', () => {
    const result = buildIndex([bug1, bug2]);
    assert.ok(result.includes("## All Bugs"));
  ...
  it('Test 23: Section "## Theo Keyword" liet ke error message keywords', () => {
    const result = buildIndex([bug1, bug2]);
    assert.ok(result.includes("## By Keyword"));
  });
});

...
describe('parseResearchFiles', () => {
  it('parse nhieu files thanh entries', () => {
    const files = [
      { filename: 'internal-a.md', content: makeResearchContent({ topic: 'Topic A' }) },
      { filename: 'RES-001-b.md', content: makeResearchContent({ source: 'external', topic: 'Topic B' }) },
    ];
...
describe('generateIndex', () => {
  it('tao INDEX voi entries', () => {
    const entries = [
      { filename: 'a.md', source: 'internal', topic: 'A', confidence: 'HIGH', created: '2026-03-25' },
      { filename: 'b.md', source: 'external', topic: 'B', confidence: 'LOW', created: '2026-03-24' },
    ];
  ...
  it('sort theo created desc (moi nhat truoc)', () => {
    const entries = [
      { filename: 'old.md', source: 'internal', topic: 'Old', confidence: 'LOW', created: '2026-03-20' },
      { filename: 'new.md', source: 'internal', topic: 'New', confidence: 'HIGH', created: '2026-03-25' },
    ];
    const index = generateIndex(entries);
  ...
  it('tao INDEX rong khi entries rong', () => {
    ...
    assert.ok(index.includes(INDEX_TITLE));
    assert.ok(index.includes('Chua co research files'));
  });
  ...
  it('end-to-end: parseResearchFiles -> generateIndex', () => {
    const files = [
      { filename: 'INT-api.md', content: makeResearchContent({ topic: 'API Design', confidence: 'HIGH' }) },
      { filename: 'RES-001-node.md', content: makeResearchContent({ source: 'external', topic: 'Node.js', confidence: 'MEDIUM' }) },
    ];
    const entries = parseResearchFiles(files);
    const index = generateIndex(entries);

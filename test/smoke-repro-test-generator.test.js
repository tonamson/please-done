...
describe('generateReproTest — template content', () => {
  it('testCode chua require node:test va node:assert/strict', () => {
    ...
    assert.ok(result.testCode.includes("require('node:test')"), 'thieu node:test import');
    assert.ok(result.testCode.includes("require('node:assert/strict')"), 'thieu node:assert/strict import');
  });
  ...
  it('testCode chua AAA comments (Arrange, Act, Assert)', () => {
    ...
    assert.ok(result.testCode.includes('// --- Arrange ---'), 'thieu Arrange comment');
    assert.ok(result.testCode.includes('// --- Act ---'), 'thieu Act comment');
    assert.ok(result.testCode.includes('// --- Assert ---'), 'thieu Assert comment');
  });
  ...
  it('testCode chua symptoms.expected va symptoms.actual trong comments', () => {
    ...
    assert.ok(result.testCode.includes('tra ve 200'), 'thieu expected trong comments');
    assert.ok(result.testCode.includes('tra ve 500'), 'thieu actual trong comments');
  });
  ...
  it('testCode chua functionName khi co, ghi "chua xac dinh" khi thieu', () => {
    ...
    assert.ok(resultWithFn.testCode.includes('handleLogin'), 'thieu functionName trong testCode');
...
describe('generateReproTest — error handling', () => {
  it('throw Error khi thieu params.symptoms', () => {
    assert.throws(
      ...
      (err) => {
        ...
        assert.ok(err.message.includes('symptoms'), `message phai chua "symptoms": ${err.message}`);
        return true;
      }
    );
  });
  ...
  it('throw Error khi thieu params.bugTitle', () => {
    assert.throws(
      () => generateReproTest({ symptoms: { expected: 'a', actual: 'b' }, filePath: 'y' }),
      (err) => {
        assert.ok(err instanceof Error);
        assert.ok(err.message.includes('bugTitle'), `message phai chua "bugTitle": ${err.message}`);
        return true;
      }
    );
  });

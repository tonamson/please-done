...
| **Cursor**         | Skills + Rules tự động    | `~/.cursor/skills/pd-*/` + `rules/pd-*.mdc` |
...
$pd-init        # Codex
/pd-init        # OpenCode
# Cursor: rules tự động load — hỏi AI "khởi tạo project theo pd-init"
```
...
| Thành phần          | Claude Code       | Codex                  | Gemini                                   | OpenCode       | Copilot              | Cursor                                         |
| ------------------- | ----------------- | ---------------------- | ---------------------------------------- | -------------- | -------------------- | ---------------------------------------------- |
| **Tên công cụ**     | Read, Write, Bash | Giữ nguyên             | read_file, write_file, run_shell_command | Giữ nguyên     | read, write, execute | read_file, write_to_file, run_terminal_command |
| **Tiền tố lệnh**    | /pd:              | $pd-                   | /pd:                                     | /pd-           | /pd:                 | Agent tự quyết                                 |
| **Định dạng skill** | .md lồng nhau     | SKILL.md + XML adapter | .md lồng nhau                            | pd-*.md phẳng  | SKILL.md             | SKILL.md (skills) + .mdc (rules)               |
| **Cấu hình MCP**    | settings.json     | config.toml (TOML)     | settings.json                            | Cấu hình riêng | instructions.md      | mcp.json                                       |
...
**Cursor** — `.cursor/rules/pd-rules-general.mdc` đã chứa quy tắc bảo mật. Thêm vào `.cursorignore`:

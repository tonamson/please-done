...
8. **`pd-evidence-collector` + `pd-fact-checker` bị bỏ sót** — 2 agents này có trong `AGENT_REGISTRY` (`resource-config.js`) + smoke tests nhưng plan A4 không nhắc. **→ Đã thêm vào A4 migration table.**

9. **D2 `TECHNICAL_STRATEGY.md` guard phụ thuộc milestone 3** — File này do `3_PROJECT_RESEARCHER.md` tạo ra. Nếu milestone 3 chưa ship mà dùng hard-block → gãy tất cả `/pd:plan`. **→ Đổi sang soft-guard (warning).**
...
| ---------------------------- | ------------- | ------------------------------- | ----------------------------------------------------------------------- |
| `pd-codebase-mapper.md`      | **Scout**     | `init`, `scan`, `new-milestone` | Mới — quét cấu trúc codebase nhanh, cập nhật `.planning/codebase/`      |
...
| `pd-planner.md`              | **Architect** | `plan`                          | Mới — agent chuyên plan cho PD phases                                   |
...
Repo có 11 agents trong registry, 8 agent files tại `commands/pd/agents/`, 7 agent files tại `.claude/agents/`. Plan định nghĩa 6 agent mới. Bảng đối chiếu:
...
| 6 agents mới (mapper, researcher, analyst, synthesizer, planner, regression)                             | Tạo bổ sung theo registry A2                                    |
...
- [ ] **Tự động Mapping**: Triệu hồi `pd-codebase-mapper` (Scout) để quét cấu trúc nhanh và cập nhật `.planning/codebase/`
...
| ----- | --------------------------- | ----------------------------------------------------------------- |
| **A** | Agent Reform                | 11 agents giữ nguyên + 6 agent mới. Sec: ✅ đã DRY bởi 4_AUDIT    |
| **B** | Platform Mapping            | Config-driven, không hardcode model, fallback tự động              |
| **C** | Parallel + Resource Guard   | 2-4 workers adaptive, backpressure, không chiếm hết tài nguyên    |
| **D** | Skill Integration           | `pd-codebase-mapper` + soft-guard strategy, workflow tự động       |
...
- **Agent count**: 11 agents hiện có (giữ nguyên) + 6 agents mới = 17 agents tổng. Không đổi tên `pd-repro-engineer`.

...
async function install(skillsDir, targetDir, options = {}) {
  ...
  log.step(1, 4, "Converting skills for Gemini CLI...");

  ensureDir(commandsDir);

  // Cleanup legacy sk directory
  ...
  for (const skill of skills) {
    ...
    fs.writeFileSync(
      path.join(commandsDir, `${skill.name}.toml`),
      converted,
      "utf8",
    );
  ...
  if (fs.existsSync(settingsFile)) {
    try {
      ...
      log.warn("settings.json invalid — will overwrite with new config");
    }
  }
  ...
  console.log("");
  log.info(
    `Skills v${options.version} — ${skills.length} skills for Gemini CLI`,
  );
...
async function uninstall(targetDir) {
  ...
  if (fs.existsSync(settingsFile)) {
    try {
      ...
      log.warn(
        `Failed to clean settings.json: ${settingsFile} (${err.message})`,
      );
    }
  }
}

...
async function install(skillsDir, targetDir, options = {}) {
  ...
  log.step(1, 4, "Converting skills for Copilot...");

  const skills = listSkillFiles(skillsSrc);

  // Clean old pd-* and legacy sk-* skill dirs
  if (fs.existsSync(skillsDestDir)) {
    const existing = fs
      .readdirSync(skillsDestDir)
      .filter((d) => d.startsWith("pd-") || d.startsWith("sk-"));
  ...
  log.step(2, 4, "Saving .pdconfig configuration...");
  ...
  if (fs.existsSync(rulesDir)) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && entry.name.endsWith(".md")) {
        fs.writeFileSync(
          path.join(rulesDestDir, entry.name),
          replaceContent(fs.readFileSync(srcPath, "utf8")),
          "utf8",
        );
        ...
        for (const sf of fs
          ...
          fs.writeFileSync(
            path.join(subDestDir, sf),
            pathOnly(fs.readFileSync(path.join(srcPath, sf), "utf8")),
            "utf8",
          );
        }
      }
  ...
  log.step(4, 4, "Updating copilot-instructions.md...");
  ...
  console.log("");
  log.info(
    `Skills v${options.version} — ${skills.length} skills for GitHub Copilot`,
  );
...
async function uninstall(targetDir) {
  ...
  // Remove pd-* and legacy sk-* skill directories

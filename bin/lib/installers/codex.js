...
async function install(skillsDir, targetDir, options = {}) {
  ...
  log.step(1, 4, "Converting skills for Codex...");
  ...
  // ─── Step 2: Copy rules into each skill dir ────────────
  ...
  log.step(3, 4, "Saving .pdconfig configuration...");
  ...
  // ─── Step 4: MCP config in config.toml ─────────────
  log.step(4, 4, "Configuring MCP servers...");
  ...
  console.log("");
  log.info(
    `Skills v${options.version} — ${skills.length} skills for Codex CLI`,
  );

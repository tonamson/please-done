...
async function install(skillsDir, targetDir, options = {}) {
  ...
  if (!commandExists('claude')) {
    log.error('Claude Code CLI chưa cài. Cài trước: https://claude.ai/download');
    process.exit(1);
  }
  ...
  const pyVersion = exec(`${pythonCmd} --version`, { ignoreError: true }).replace('Python ', '');
  ...
  exec(`cd "${skillsDir}" && git submodule update --init --recursive`, { ignoreError: true });
  ...
  try {
    exec(`claude mcp add --scope user fastcode -- "${pythonPath}" "${mcpScript}"`);
  ...
  if (commandExists('npx')) {
    ...
    try {
      exec('claude mcp add --scope user context7 -- npx -y @upstash/context7-mcp@latest');
